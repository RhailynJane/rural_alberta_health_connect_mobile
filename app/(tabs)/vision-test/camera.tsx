...
import { Camera, runAtTargetFps, useCameraDevice, useCameraPermission } from "react-native-vision-camera";
...
export default function VisionCameraScreen() {
  ...
  const { setCapturedImage, setCapturedDetections, setFrameDimensions: setSessionFrameDimensions } = useVisionSession();
  ...
  const frameProcessor = useMemo(() => {
    ...
    const workletFunction = (frame: any) => {
      ...
      if (!didLogFirstFrame) {
        ...
        try { logFrameError("First frame received by worklet"); } catch {}
      }
      ...
      if (tfliteModel) {
        runAtTargetFps(10, () => {
          ...
          try {
            ...
            const outputs = tfliteModel.runSync ? tfliteModel.runSync([resized]) : undefined;
            if (!outputs) {
              throw new Error("TFLite runSync returned no outputs (model not available on worklet?)");
            }
            ...
            const boxes = Array.isArray(rawBoxes?.[0]) && Array.isArray(rawBoxes?.[0][0]) ? rawBoxes[0] : rawBoxes;
            const classes = Array.isArray(rawClasses?.[0]) && !Array.isArray(rawClasses?.[0][0]) ? rawClasses[0] : rawClasses;
            const scores = Array.isArray(rawScores?.[0]) && !Array.isArray(rawScores?.[0][0]) ? rawScores[0] : rawScores;
            ...
            const palette = ["#FF6B6B", "#4ECDC4", "#95E1D3", "#FFD93D", "#A8E6CF", "#2A7DE1"]; 
            ...
            function mapClassIdToIndex80(classId: number) {
              ...
              const missingBefore = MISSING_IDS.reduce((c, m) => c + (m <= id ? 1 : 0), 0);
            ...
            for (let i = 0; i < numDetections; i++) {
              ...
              if (confidence >= CONF_THRESHOLD) {
                ...
                if (maybeRow && (Array.isArray(maybeRow) || typeof maybeRow?.length === 'number')) {
                ...
                if (areaRatio > LARGE_BOX_RATIO && confidence < LARGE_BOX_MIN_CONF) {
                  continue;
                }
            ...
            const sorted = newDetections.sort((a, b) => b.confidence - a.confidence);
            ...
            for (const label in byLabel) {
              ...
              for (let i = 0; i < list.length; i++) {
                ...
                for (let j = 0; j < kept.length; j++) {
                  if (iou(cand, kept[j]) > IOU_THRESHOLD) { ok = false; break; }
                }
            ...
            for (let t = 0; t < tracks.length; t++) {
              ...
              for (let i = 0; i < selected.length; i++) {
                ...
                if (overlap > bestIou) { bestIou = overlap; bestIdx = i; }
              }
              if (bestIdx >= 0 && bestIou >= MATCH_IOU) {
                ...
                const sw = GEO_ALPHA * cand.width + (1 - GEO_ALPHA) * tracks[t].width;
                const sh = GEO_ALPHA * cand.height + (1 - GEO_ALPHA) * tracks[t].height;
                // Smooth confidence (EMA) with hysteresis consideration
                const targetConf = (cand.confidence >= KEEP_THRESHOLD || cand.confidence >= ADD_THRESHOLD)
                  ? cand.confidence
                  : Math.max(cand.confidence, tracks[t].confidence * 0.9);
                const sc = CONF_ALPHA * targetConf + (1 - CONF_ALPHA) * tracks[t].confidence;

                tracks[t] = { ...cand, x: sx, y: sy, width: sw, height: sh, confidence: sc, misses: 0 };
                used[bestIdx] = true;
              } else {
                // No match this frame
                tracks[t].misses += 1;
            ...
            const output = tracks.slice(0, MAX_DETECTIONS).map(tr => ({
              label: tr.label,
        ...
        if (!didWarnMissingModel) {
          ...
          try { logFrameError('TFLite model not yet available inside worklet'); } catch {}
        }
      }
    };
  ...
  }, [isCapturing, frameDimensions.width, saveFrameDimensions, tflitePtr, resize, updateLatestDetections, logFrameError]);

  const handleCapture = async () => {
    if (!cameraRef.current) return;
    try {
      // Snapshot last known detections (prefer current, else last non-empty)
      const snapshot = (latestDetections && latestDetections.length > 0)
        ? latestDetections
      ...
      console.log("Photo captured:", photo.path, "with", currentDetections.length, "detections");
  ...
  if (!hasPermission) {
    return (
      <SafeAreaView style={styles.safeArea}>
        <CurvedBackground>
          <CurvedHeader title="Vision Test - Camera" height={120} showLogo={false} screenType="signin" bottomSpacing={0} />
          <View style={styles.centerBox}>
            <Ionicons name="camera" size={48} color="#666" />
            <Text style={[styles.title, { fontFamily: FONTS.BarlowSemiCondensed }]}>Camera permission required</Text>
            <TouchableOpacity style={styles.primaryBtn} onPress={requestPermission}>
              <Text style={[styles.primaryBtnText, { fontFamily: FONTS.BarlowSemiCondensed }]}>Grant Permission</Text>
            </TouchableOpacity>
          </View>
          <BottomNavigation />
        </CurvedBackground>
      ...
      <SafeAreaView style={styles.safeArea}>
        <CurvedBackground>
          ...
          <View style={styles.centerBox}>
            ...
            <Text style={[styles.title, { fontFamily: FONTS.BarlowSemiCondensed }]}>Loading vision model...</Text>
          </View>
          <BottomNavigation />
        </CurvedBackground>
      </SafeAreaView>
    ...
    <View style={styles.fullScreen}>
      ...
      <View style={StyleSheet.absoluteFill} pointerEvents="none">
        {latestDetections.map((detection, index) => (
          ...
          >
            <View style={[styles.labelBox, { backgroundColor: detection.color }]}>
      ...
      <View style={styles.statusOverlay}>
        ...
        <Text style={[styles.statusSubtext, { fontFamily: FONTS.BarlowSemiCondensed }]}>
      ...
      <View style={styles.controlsContainer}>
        <TouchableOpacity style={styles.captureButton} onPress={handleCapture}>
          ...
          <Text style={[styles.buttonText, { fontFamily: FONTS.BarlowSemiCondensed }]}>Capture</Text>
        </TouchableOpacity>
      </View>
...
const styles = StyleSheet.create({
  ...
  primaryBtn: { backgroundColor: "#2A7DE1", paddingVertical: 14, paddingHorizontal: 20, borderRadius: 12, alignSelf: "center", marginTop: 12 },
  primaryBtnText: { color: "white", fontSize: 16, fontWeight: "600" },
  statusOverlay: { position: "absolute", top: 60, left: 16, right: 16, backgroundColor: "rgba(0, 0, 0, 0.6)", padding: 12, borderRadius: 8, alignItems: "center" },
  ...
  controlsContainer: { position: "absolute", bottom: 100, left: 0, right: 0, alignItems: "center" },
  captureButton: { alignItems: "center", padding: 16 },
  buttonText: { color: "white", fontSize: 16, fontWeight: "600", marginTop: 4, textShadowColor: "rgba(0, 0, 0, 0.75)", textShadowOffset: { width: 0, height: 1 }, textShadowRadius: 3 },
  detectionBox: {
    position: "absolute",
    borderWidth: 3,
    borderRadius: 4,
