...
function cleanLLMResponse(text: string): string {
  ...
  console.log("[LLM CLEAN] Raw response length:", text.length);
  console.log("[LLM CLEAN] First 300 chars:", text.slice(0, 300));
  // Remove echoed prompt, markdown, and extra whitespace, but be less aggressive
  let cleaned = text
  ...
  console.log("[LLM CLEAN] Cleaned response length:", cleaned.length);
  console.log("[LLM CLEAN] First 300 chars of cleaned:", cleaned.slice(0, 300));
  // If cleaning strips everything but raw is not empty, fallback to raw (with warning)
  if (!cleaned && text.trim().length > 0) {
    console.warn("[LLM CLEAN] Cleaned response is empty but raw is not. Falling back to raw response.");
    return text.trim();
  }
  return cleaned;
}
...
function renderAssessmentCards(text: string | null) {
  ...
  console.log("=== Parsing Assessment Response ===");
  console.log("Raw text length:", text.length);
  console.log("First 200 chars:", text.substring(0, 200));
  
  const lines = text.split(/\r?\n/).map((l) => l.trim());
  type SectionKey = 'SUMMARY' | 'VISUAL ASSESSMENT' | 'FIRST AID' | 'RED FLAGS' | 'WHAT NEXT' | 'OTHER';
  const wantedOrder: SectionKey[] = ['SUMMARY', 'VISUAL ASSESSMENT', 'FIRST AID', 'RED FLAGS', 'WHAT NEXT'];
  ...
  const isHeader = (l: string): SectionKey | null => {
    ...
    if (/^visual\s+assessment\s*:?$/i.test(lower)) {
      console.log("  -> Matched VISUAL ASSESSMENT");
      return 'VISUAL ASSESSMENT';
    }
    if (/^first[\s-]?aid\s*:?$/i.test(lower)) {
      console.log("  -> Matched FIRST AID");
      return 'FIRST AID';
    }
    if (/^red\s+flags\s*:?$/i.test(lower)) {
      console.log("  -> Matched RED FLAGS");
      return 'RED FLAGS';
    }
    if (/^what[\s-]?next\s*:?$/i.test(lower)) {
      console.log("  -> Matched WHAT NEXT");
      return 'WHAT NEXT';
    }
    return null;
  };
  ...
  const Card = ({ title, items, icon }: { title: string; items: string[]; icon: React.ReactNode }) => (
    <View style={styles.responseCard}>
      <View style={styles.responseHeader}>
        {icon}
        <Text style={[styles.responseTitle, { fontFamily: FONTS.BarlowSemiCondensed }]}>{title}</Text>
      </View>
      {items.map((it, idx) => (
        <Text key={idx} style={[styles.responseText, { fontFamily: FONTS.BarlowSemiCondensed, marginBottom: 6 }]}>
          {it}
        </Text>
      ))}
    </View>
...
export default function VisionReviewScreen() {
  ...
  const { capturedImage, capturedDetections, frameDimensions, reset } = useVisionSession();
  ...
  // Monitor LLM state changes to prevent unexpected resets
  useEffect(() => {
    console.log("LLM state changed:", {
      isReady: llm.isReady,
    ...
    // If generation stopped but we're still marked as analyzing, sync the state
    if (isAnalyzing && !llm.isGenerating) {
      console.log("Generation finished, syncing state");
    }
  }, [llm.isReady, llm.isGenerating, llm.error, llm.response, isAnalyzing]);

  const handleStartOver = () => {
    // Prevent reset during analysis to avoid interrupting the LLM
    if (llm.isGenerating || isAnalyzing) {
      console.log("Cannot start over while analysis is in progress");
      return;
    }
    reset();
...
Begin your response with "SUMMARY:" and nothing else.`;
        ...
        console.log("LLM generation complete. Response length:", llm.response?.length || 0);
        console.log("[LLM DEBUG] llm.response:", JSON.stringify(llm.response));
        console.log("[LLM DEBUG] llm.error:", llm.error);
        // If responseLength is nonzero but response is empty, log a warning and show user error
        if ((llm.response?.length ?? 0) === 0 && (llm as any).responseLength > 0) {
          console.warn("[LLM ENGINE BUG] responseLength > 0 but llm.response is empty!");
          setLlmResponse("AI engine error: The model generated a response but it could not be retrieved. Please try again or restart the app.");
          setIsAnalyzing(false);
          return;
        }
        let rawResponse = llm.response || "";
        // Strip everything before "SUMMARY:" if the LLM echoed the prompt
        const summaryIndex = rawResponse.search(/SUMMARY\s*:/i);
        if (summaryIndex > 0) {
          console.log("Stripping echoed prompt before SUMMARY at position:", summaryIndex);
          rawResponse = rawResponse.substring(summaryIndex);
        }
        // Clean and fallback if needed
        cleanedResponse = cleanLLMResponse(rawResponse);
        if (cleanedResponse && cleanedResponse.trim().length > 0) {
          setLlmResponse(cleanedResponse);
          requestAnimationFrame(() => {
            scrollViewRef.current?.scrollTo({ y: aiAssessmentY - 20, animated: true });
          });
          break;
        }
      } catch (error) {
    ...
    if (!cleanedResponse || cleanedResponse.trim().length === 0) {
      setLlmResponse(
        "The AI assessment completed but returned no content. Please try again with a different description."
      );
    }
    setIsAnalyzing(false);
  };

  const handleBackToCamera = () => router.replace({ pathname: "/(tabs)/vision-test/camera" } as any);
          ...
          <View style={styles.contentArea}>
            ...
            >
              <View style={styles.contentSection}>
                <Text style={[styles.sectionTitle, { fontFamily: FONTS.BarlowSemiCondensed }]}>Captured Image</Text>

                {!hasPreview ? (
                  <View style={styles.placeholderCard}>
                    <Ionicons name="images" size={24} color="#2A7DE1" />
                    <Text style={[styles.placeholderText, { fontFamily: FONTS.BarlowSemiCondensed }]}>No image captured yet</Text>
                    <TouchableOpacity style={styles.primaryBtn} onPress={handleBackToCamera}>
                      <Text style={[styles.primaryBtnText, { fontFamily: FONTS.BarlowSemiCondensed }]}>Back to Camera</Text>
                    </TouchableOpacity>
                  </View>
                ) : (
                  <>
                    ...
                    <View style={styles.detectionsSummary}>
                      <Text style={[styles.detectionsSummaryTitle, { fontFamily: FONTS.BarlowSemiCondensed }]}>Detected Objects:</Text>
                      {capturedDetections && capturedDetections.length > 0 ? (
                        capturedDetections.map((det, idx) => (
                          <Text key={idx} style={[styles.detectionItem, { fontFamily: FONTS.BarlowSemiCondensed }]}>• {det.label} ({Math.round(det.confidence * 100)}%)</Text>
                        ))
                      ) : (
                        <Text style={[styles.detectionItem, { fontFamily: FONTS.BarlowSemiCondensed }]}>No objects detected</Text>
                      )}
                    </View>
                  </>
                )}
                ...
                <View style={styles.statusCard}>
                  <View style={styles.statusRow}>
                    <Text style={[styles.statusLabel, { fontFamily: FONTS.BarlowSemiCondensed }]}>Model Status:</Text>
                    <View style={styles.statusBadge}>
                      <View style={styles.statusDot} />
                      <Text style={[styles.statusValue, { fontFamily: FONTS.BarlowSemiCondensed }]}>
                        {llm.isGenerating ? 'Analyzing…' : (llm.isReady ? 'Ready' : 'Loading…')}
                      </Text>
                    </View>
                  </View>
                  <View style={styles.statusRow}>
                    <Text style={[styles.statusLabel, { fontFamily: FONTS.BarlowSemiCondensed }]}>Model:</Text>
                    <Text style={[styles.statusValue, { fontFamily: FONTS.BarlowSemiCondensed }]}>Llama 3.2 1B (on-device)</Text>
                  </View>
                </View>

                <Text style={[styles.inputLabel, { fontFamily: FONTS.BarlowSemiCondensed }]}>Describe what you&apos;re experiencing:</Text>
                <TextInput
                ...
                <View style={styles.actionButtons}>
                  ...
                  >
                    ...
                    <Text style={[styles.resetButtonText, { fontFamily: FONTS.BarlowSemiCondensed }]}>Start Over</Text>
                  </TouchableOpacity>

                  <TouchableOpacity
                    style={[styles.analyzeButton, (!userDescription.trim() || !llm.isReady || llm.isGenerating || isAnalyzing) && styles.analyzeButtonDisabled]}
                    onPress={handleAnalyzeWithAI}
                    disabled={!userDescription.trim() || !llm.isReady || llm.isGenerating || isAnalyzing}
                  >
                    ...
                    <Text style={[styles.analyzeButtonText, { fontFamily: FONTS.BarlowSemiCondensed }]}>
                      {(llm.isGenerating || isAnalyzing) ? 'Analyzing...' : !llm.isReady ? 'Loading AI...' : 'Analyze with AI'}
                    </Text>
                  </TouchableOpacity>
                </View>

                {(isAnalyzing || llm.isGenerating) && (
                  <View style={styles.assessmentSection} ref={aiAssessmentRef} onLayout={(e) => setAiAssessmentY(e.nativeEvent.layout.y)}>
                    <View style={styles.analyzingCard}>
                      <Ionicons name="information-circle" size={20} color="#2A7DE1" />
                      <View style={{ flex: 1 }}>
                        <Text style={[styles.analyzingTitle, { fontFamily: FONTS.BarlowSemiCondensed }]}>Analysis in Progress</Text>
                        <Text style={[styles.analyzingDescription, { fontFamily: FONTS.BarlowSemiCondensed }]}>We are analyzing your image and description. This may take a few moments. The results will appear here as soon as the assessment is complete.</Text>
                      </View>
                    </View>
                  </View>
                )}
                ...
                {!llm.isGenerating && llmResponse && (
                  <View style={styles.assessmentSection} ref={aiAssessmentRef} onLayout={(e) => setAiAssessmentY(e.nativeEvent.layout.y)}>
                    ...
                    <View className="disclaimerBanner" style={styles.disclaimerBanner}>
                      <Ionicons name="information-circle" size={18} color="#FF6B35" />
                      <Text style={[styles.disclaimerText, { fontFamily: FONTS.BarlowSemiCondensed }]}>Testing Only - Not for Actual Medical Use</Text>
                    </View>
                    <TouchableOpacity style={styles.newAssessmentButton} onPress={handleStartOver}>
                      <Ionicons name="refresh" size={20} color="#2A7DE1" />
                      <Text style={[styles.newAssessmentButtonText, { fontFamily: FONTS.BarlowSemiCondensed }]}>New Assessment</Text>
                    </TouchableOpacity>
                  </View>
                )}
                ...
                {!llm.isGenerating && !llmResponse && llm.error && (
                  <View style={[styles.assessmentSection, { marginTop: 12 }]}>
                    <View style={[styles.disclaimerBanner, { backgroundColor: '#FDECEC', borderLeftColor: '#DC3545' }]}>
                      <Ionicons name="alert-circle" size={18} color="#DC3545" />
                      <Text style={[styles.disclaimerText, { color: '#8B0000', fontFamily: FONTS.BarlowSemiCondensed }]}>
...
const styles = StyleSheet.create({
  ...
  sectionTitle: { fontSize: 24, fontWeight: "700", color: "#1A1A1A", marginBottom: 16 },
  placeholderCard: { alignItems: 'center', gap: 12, padding: 20, backgroundColor: '#F0F8FF', borderWidth: 1, borderColor: '#2A7DE1', borderRadius: 12, marginBottom: 20 },
  placeholderText: { fontSize: 14, color: '#1A1A1A' },
  primaryBtn: { backgroundColor: "#2A7DE1", paddingVertical: 14, borderRadius: 12, alignItems: "center", paddingHorizontal: 16 },
  primaryBtnText: { color: "white", fontSize: 16, fontWeight: "600" },
  imagePreviewContainer: { width: "100%", height: 300, backgroundColor: "#F5F5F5", borderRadius: 12, overflow: "hidden", marginBottom: 20, position: "relative" },
  previewImage: { width: "100%", height: "100%" },
  boundingBox: { position: "absolute", borderWidth: 2, borderColor: "#FF6B6B", backgroundColor: "rgba(255, 107, 107, 0.1)" },
  boundingBoxLabel: { position: "absolute", top: -20, left: 0, backgroundColor: "#FF6B6B", color: "white", fontSize: 12, fontWeight: "600", paddingHorizontal: 8, paddingVertical: 2, borderRadius: 4 },
  detectionsSummary: { backgroundColor: "white", borderWidth: 1, borderColor: "#E0E0E0", borderRadius: 12, padding: 16, marginBottom: 20 },
  detectionsSummaryTitle: { fontSize: 16, fontWeight: "600", color: "#1A1A1A", marginBottom: 12 },
  detectionItem: { fontSize: 14, color: "#666", marginBottom: 6, lineHeight: 20 },
  inputLabel: { fontSize: 16, fontWeight: "600", color: "#1A1A1A", marginBottom: 8 },
  descriptionInput: { backgroundColor: "#FFFFFF", borderWidth: 1, borderColor: "#E0E0E0", borderRadius: 12, padding: 16, fontSize: 16, color: "#1A1A1A", minHeight: 100, textAlignVertical: "top", marginBottom: 20 },
  actionButtons: { flexDirection: "row", gap: 12 },
  resetButton: { flex: 1, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8, backgroundColor: "white", borderWidth: 2, borderColor: "#DC3545", paddingVertical: 16, borderRadius: 12 },
  resetButtonText: { color: "#DC3545", fontSize: 16, fontWeight: "600" },
  analyzeButton: { flex: 2, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8, backgroundColor: "#2A7DE1", paddingVertical: 16, borderRadius: 12 },
  analyzeButtonDisabled: { backgroundColor: "#A0A0A0" },
  analyzeButtonText: { color: "white", fontSize: 16, fontWeight: "600" },
  statusCard: { backgroundColor: "white", padding: 16, borderRadius: 12, marginBottom: 20, borderWidth: 1, borderColor: "#E9ECEF", shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.1, shadowRadius: 3, elevation: 2 },
  statusRow: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: 8 },
  statusLabel: { fontSize: 14, color: "#666" },
  statusValue: { fontSize: 14, color: "#1A1A1A", fontWeight: "600" },
  statusBadge: { flexDirection: "row", alignItems: "center" },
  statusDot: { width: 8, height: 8, borderRadius: 4, backgroundColor: "#28A745", marginRight: 6 },
  assessmentSection: { marginTop: 24 },
  analyzingCard: { flexDirection: 'row', gap: 12, alignItems: 'flex-start', backgroundColor: '#F0F8FF', borderWidth: 1, borderColor: '#2A7DE1', borderRadius: 12, padding: 16 },
  analyzingTitle: { fontSize: 16, fontWeight: '700', color: '#1A1A1A', marginBottom: 6 },
  analyzingDescription: { fontSize: 14, color: '#333', lineHeight: 20 },
  responseCard: { backgroundColor: "#F0F8FF", padding: 16, borderRadius: 12, marginBottom: 20, borderWidth: 1, borderColor: "#2A7DE1" },
  responseHeader: { flexDirection: "row", alignItems: "center", marginBottom: 12, paddingBottom: 8, borderBottomWidth: 1, borderBottomColor: "#D1E8FF" },
  responseTitle: { fontSize: 16, fontWeight: "600", color: "#1A1A1A", marginLeft: 8 },
  responseText: { fontSize: 15, color: "#1A1A1A", lineHeight: 22 },
  disclaimerBanner: { flexDirection: "row", alignItems: "center", gap: 8, backgroundColor: "#FFF3E0", borderLeftWidth: 4, borderLeftColor: "#FF6B35", padding: 12, borderRadius: 8, marginBottom: 16 },
  disclaimerText: { flex: 1, fontSize: 14, color: "#8B4513", fontWeight: "600" },
  newAssessmentButton: { flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8, backgroundColor: "white", borderWidth: 2, borderColor: "#2A7DE1", paddingVertical: 14, borderRadius: 12 },
  newAssessmentButtonText: { color: "#2A7DE1", fontSize: 16, fontWeight: "600" },
});